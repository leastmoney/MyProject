...
<head>
	...
	<!-- <script type="text/javascript">
	  $(function(){
		...
		$.ajax({
			   type: "POST",
			   url: "userAction_ajaxManager",
			   data: "id="+$(this).val(),
			   success: function(data){
				   $("#manager_id")[0].length = 1;
					$(data).each(function(){
						var option = $("<option value='"+this.userInfo.id+"'>"+this.userInfo.name+"</option>")
...
<script type="text/javascript">
	function leaderid(){
	    //在#dept_id选择器的标签下的option标签选中的选项
	    var deptId = $("#dept_id>option:selected");

	   /* $.ajax({
			url:"userAction_ajaxManager?deptId="+deptId.val(),
            //返回的是存文本类型
            dataType:"text",
			type:"post",
            success:function(data){
			    //写了 dataType:"text"的话是纯文本文档,需要解析为json格式
            var datas = eval("("+data+")");
//            每次选别的的时候请空
                $("#leader")[0].length = 1;
                $(datas).each(function(){
                    var option = $("<option value='"+this.id+"'>"+this.userInfoName+"</option>")
        ...
        $.ajax({
            ...
            success:function(data){
                $("#leader")[0].length = 1;
                $(data).each(function(){
                    $("#leader").append("<option value='"+this.id+"'>"+this.userInfoName+"</option>");
                })
               /*for (var i = 0;i<data.length;i++){
                   $("#leader").append("<option value='"+data[i].id+"'>"+data[i].userInfoName+"</option>");
			   }*/
            }
        })
	}
		...
		<table class="commonTable" cellspacing="1">
       		<tr>
	            ...
	            <td class="tableContent">
	            	<s:select name="dept.id" list="deptList" 
	            		...
	            		headerKey="" headerValue="--请选择--" onchange="leaderid()"
	            	></s:select>
	            </td>
	        </tr>
        	<tr>
        	...
        	<tr>
	            ...
	            <td class="tableContent">
	            	...
	            	></s:select>   --%>
	            	<!-- <select id="manager_id" name="userInfo.manager.id">
	            		<option value="">--请选择--</option>
	            	</select>  -->
	            	<select name="userInfo.manager.id" id="leader">
						<option value="">--请选择--</option>
					</select>
	            </td>
	        </tr>		
