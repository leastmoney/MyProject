...
import cn.itcast.jx.util.PoiUtil;
...
/*//		创建一个list集合用来存放所有的cp
		ArrayList<ContractProduct> list1 = new ArrayList<>();
//		把set中所有的元素放进去
		list1.addAll(set);
//		创建一个set集合用来存放厂家的名字
		Set<String> oldFactoryName = new HashSet<>();
//遍历list1,把不同的厂家名字放在set集合中
		for (ContractProduct cp : list1) {
			oldFactoryName.add(cp.getFactoryName());
		}
//		创建一个集合用来放有序的货物
		ArrayList<ContractProduct> oList = new ArrayList<>();
//		遍历oldFactoryName集合,
		for (String name : oldFactoryName) {
//			遍历list1集合,用来取出货物.因为一个厂家不一定只有一个货物
			for (int i = 0;i<list1.size();i++){
				ContractProduct cp = list1.get(i);
		...
		Collections.sort(oList, new Comparator<ContractProduct>() {
			public int compare(ContractProduct o1, ContractProduct o2) {
				return o1.getFactoryName().compareTo(o2.getFactoryName());
			}
		});
