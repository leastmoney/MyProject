...
<%-- <c:forEach items="${_CURRENT_USER.roles}" var="role">
    	...
    	<c:if test="${fn:contains(userRoleStr,o.name)}">checked</c:if>
    		<c:if test="${fn:contains(module.id,'5021') }">
    			<li id="view"><a href="javascript:toView()">查看</a></li>
    		</c:if>
    		<c:if test="${fn:contains(module.id,'5022') }">
    		<li id="new"><a href="#" onclick="formSubmit('userAction_tocreate','_self');this.blur();">新增</a></li>
    		</c:if>
    		<c:if test="${fn:contains(module.id,'5023') }">
    		<li id="update"><a href="#" onclick="javascript:toUpdate()">修改</a></li>
    		</c:if>
    		<c:if test="${fn:contains(module.id,'5024') }">
    		<li id="delete"><a href="#" onclick="javascript:toDelete()">删除</a></li>
    		</c:if>
    	</c:forEach>
    </c:forEach> --%>
...
${links}
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" align="left">
		...
		<td>${o.state==1?"<font color='green'>启用</font>":"<font color='red'>禁用</font>"}</td>
	</tr>
	</c:forEach>
